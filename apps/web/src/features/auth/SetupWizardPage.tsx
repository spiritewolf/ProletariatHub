...
import { useSetup } from '@/features/auth/useSetup';
...
export function SetupWizardPage(): React.ReactElement {
  ...
  async function loadRecruits() {
    const comrades = await fetchComrades();
    setRecruits(comrades);
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const data = await fetchComrades();
        if (!cancelled) {
          setRecruits(data);
        }
      } catch {
  ...
  }, [step, authenticatedComrade?.isAdmin, fetchComrades]);
  ...
  async function saveHub(e: FormEvent) {
    ...
    setPending(true);
    try {
      await saveHubName(parsed.data.name);
  ...
  return (
    <DashboardShell topBar={<PageChromeTopBar title="Hub setup" />}>
