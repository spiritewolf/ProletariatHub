...
export function DocsServicesTab({ onBanner }: DocsServicesTabProps): React.ReactElement {
  const queryClient = useQueryClient();
  const {
    fetchServiceTiles,
    createServiceTile,
    updateServiceTile,
    deleteServiceTile: deleteServiceTileFromApi,
  } = useDocs();
  ...
  const tilesQuery = useQuery({
    ...
    queryFn: fetchServiceTiles,
  });

  const createMutation = useMutation({
    mutationFn: createServiceTile,
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteServiceTileFromApi,
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: docsQueryKeys.serviceTiles });
    },
  });
