...
export function DocsCredentialsTab({ onBanner }: DocsCredentialsTabProps) {
  ...
  const createCredentialMutation = useMutation({
    mutationFn: async (payload: {
      ...
      apiJsonValidated(DocsApiPaths.credentials, createCredentialResponseSchema, {
        method: 'POST',
        json: payload,
      }),
    onSuccess: async () => {
  ...
  const copySecret = async (id: string) => {
    ...
    setCopyingId(id);
    try {
      const res = await apiJsonValidated(
        DocsApiPaths.credentialReveal(id),
        revealCredentialResponseSchema,
        { method: 'POST', json: {} },
      );
      await navigator.clipboard.writeText(res.secretPlain);
    } catch (err) {
      onBanner(err instanceof Error ? err.message : DocsPageCopy.loadError);
    } finally {
      setCopyingId(null);
