...
db_reset:
	$(COMPOSE) run --rm -T api sh -c 'cd packages/db && pnpm exec prisma migrate reset --force'

db_studio:
	$(COMPOSE) run --rm -T -p 5555:5555 api sh -c 'cd packages/db && pnpm exec prisma studio --hostname 0.0.0.0 --port 5555'
